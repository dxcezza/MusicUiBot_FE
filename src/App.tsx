...
function App() {
  ...
  const handleSearch = async () => { // Добавляем async
    if (!searchQuery.trim()) return;
    setIsLoading(true);
    try {
      const response = await fetch(`${apiUrl}/search?query=${encodeURIComponent(searchQuery)}`); // Используем apiUrl
  ...
  const playTrack = async (track: Track) => { // Добавляем async
    setCurrentTrack(track);
    const audioUrl = `${apiUrl}/get_audio/${track.videoId}`; // Используем apiUrl
    ...
    if (audioContextRef.current?.state === 'suspended') {
      await audioContextRef.current.resume(); // Используем await
    }
  };

  const downloadTrack = (track: Track) => {
    window.location.href = `${apiUrl}/get_audio/${track.videoId}`; // Используем apiUrl
  };

  const togglePlay = async () => { // Добавляем async
    if (audioRef.current) {
      // Инициализируем AudioContext, если нужно
      initializeAudioContext();
